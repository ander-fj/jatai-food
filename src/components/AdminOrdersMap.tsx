  ...
  useEffect(() => {
    ...
    if (isTrackingPage && selectedOrderId) {
      ...
      const filteredOrders = allOrdersToProcess.filter((order: Order) => {
        ...
        const isFinalized = order.status === 'Entregue' || order.status === 'Cancelado' || order.status === 'Pago';
        ...
        if (orderFilter === 'all') return !isFinalized;
        if (orderFilter === 'assigned') return isAssigned && !isFinalized;
        if (orderFilter === 'unassigned') return !isAssigned && !isFinalized;
        if (orderFilter === 'delivered') return order.status === 'Entregue';
        return false;
      });
      ...
      const processOrderMarkers = async (orderData: Order) => {
        ...
        // Sempre geocodificar o endereço para garantir a posição correta no mapa de roteirização.
        // Isso evita usar coordenadas antigas ou de pedidos anteriores.
        if (orderData.address && orderData.address !== 'Consumo no local') {
