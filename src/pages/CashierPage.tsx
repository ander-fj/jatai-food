...
const CashierPage: React.FC<CashierPageProps> = ({ orders, updateOrderStatus }) => {
  ...
  const { salaoOrders, outrosOrders, totalSalao, totalOutros } = useMemo(() => {
    const salao: Order[] = [];
    const outros: Order[] = [];
    let salaoTotal = 0;
    let outrosTotal = 0;
    ...
    for (const order of unpaidOrders) {
      ...
      const idMatch = (order.id || '').includes(searchTerm);

      if (order.address === 'Consumo no local') {
        if (nameMatch || idMatch) {
          salao.push(order);
          salaoTotal += order.total || 0;
        }
      } else {
        const phoneMatch = (order.phone || '').includes(searchTerm);
        if (nameMatch || idMatch || phoneMatch) {
          outros.push(order);
          outrosTotal += order.total || 0;
        }
      }
    }
    ...
    return { 
      salaoOrders: salao, 
      outrosOrders: outros, 
      totalSalao: salaoTotal, 
      totalOutros: outrosTotal 
    };
  ...
  const handleConfirmPayment = (method: PaymentMethod) => {
    if (selectedOrderForPayment) {
      updateOrderStatus(selectedOrderForPayment, 'Entregue', method);
